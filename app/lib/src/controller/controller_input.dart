...
class Rotated extends ControllerEvent {
  const Rotated(this.detents);

  final int detents;

  @override
  String toString() => 'Rotated($detents)';
}
...
class MockControllerInput implements ControllerInput {
  ...
  void rotate(int detents) => _events.add(Rotated(detents));

  void press() => _events.add(const Pressed());
